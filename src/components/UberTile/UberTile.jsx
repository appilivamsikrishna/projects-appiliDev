...
class UberTile extends React.Component {
  render() {
    ...
    if (item.background_color) {
      backgroundImageStyle.backgroundColor = item.background_color;
    }
    if (item.text_color) {
    ...
    if (item.background_logo) {
      ...
      backgroundImageStyle.backgroundSize = item.background_logo_size
        ? item.background_logo_size
        : "50%";
    }
    if (item.background_picture) {
      ...
      const backgroundSizeSuffix = item.background_picture_size
        ? item.background_picture_size
        : "cover";
      backgroundImageStyle.backgroundSize = `${backgroundSizePrefix}${backgroundSizeSuffix}`;
    }
    if (item.title && (item.background_logo || item.background_picture)) {
      backgroundImageStyle.opacity = "0.2";
    }

    return (
      <div
        className={`UberTile${item.link ? " withLink" : ""}`}
        style={tileStyle}
      >
        <a
        ...
        >
          ...
          />
          {item.description && (
            <div className="UberTile-description" style={descriptionStyle}>
              <span>{item.description}</span>
            </div>
          )}
          <div className="UberTile-title" style={titleStyle}>
